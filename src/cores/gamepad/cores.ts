...
/** 摇杆轴索引：左摇杆 XY=0/1，右摇杆 XY=2/3（滚动仅用右摇杆 Y） */
...
/** 摇杆死区：低于此幅度的漂移视为 0，避免手柄回中不准导致光标自走 */
 ...
 * @param deadzone 死区阈值，默认 GAMEPAD_DEADZONE
 * @returns 过滤后的轴值
 */
export function applyDeadzone(value: number, deadzone: number = GAMEPAD_DEADZONE): number {
 ...
 * @param resolvePreset 当前预设问询，切换后下帧即生效
...
export function startGamepadPolling(
  ...
  const tick = (): void => {
    if (stopped) return;
    if (isActive()) {
      const snapshot = readFirstGamepad();
      const buttonMap = buildButtonMap(resolvePreset(), DPAD_INDICES);
...
function readFirstGamepad(): GamepadSnapshot {
  ...
  const leftStick: GamepadStickState = {
    x: applyDeadzone(active.axes[AXIS_INDEX_LEFT_X] ?? 0),
    y: applyDeadzone(active.axes[AXIS_INDEX_LEFT_Y] ?? 0),
  };
  const rightStick: GamepadStickState = {
    x: applyDeadzone(active.axes[AXIS_INDEX_RIGHT_X] ?? 0),
    y: applyDeadzone(active.axes[AXIS_INDEX_RIGHT_Y] ?? 0),
  };
