...
const PLUGIN_NAME = "Local Speech Recognition";
...
class SidebarView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    this.mounted = mountComponent(this.contentEl, SidebarRoot, { plugin: this.plugin });
  }
 ...
 * @param plugin 插件实例
 */
async function activateSidebar(plugin: LocalSpeechRecognitionPlugin): Promise<void> {
 ...
 * @param plugin 插件实例
 */
export async function initSidebar(plugin: LocalSpeechRecognitionPlugin): Promise<void> {
  plugin.registerView(SIDEBAR_VIEW_TYPE, (leaf) => new SidebarView(leaf, plugin));
  plugin.addRibbonIcon("mic-vocal", PLUGIN_NAME, () => void activateSidebar(plugin));
}
