...
- 双后端存储：`global-store.ts`（全局 IndexedDB，数据库名 `local-speech-recognition`、对象仓库 `lexicon`，`keyPath: "id"` + `autoIncrement`，全仓库共享）与 `vault-store.ts`（仓库独立文件 `<vault>/.obsidian/plugins/<id>/lexicon.json`，`{ version, nextId, entries }` 结构 + `nextId` 持久化防 id 重用，内存单源 + 落盘串行队列，仅当前仓库可见），两后端数据相互独立不互相同步；`file-format.ts` 的 `normalizeStoredEntry` 为两后端共用的读取归一化口径
